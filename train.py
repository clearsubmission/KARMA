...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  KARMA-T (Hybrid)  |  Dataset: {args.dataset}  |  Device: {device}")
    print(f"{'='*60}")
    ...
    filter_dict = build_filter_dict_normalized(all_quads, tkgdata.t_min, tkgdata.t_range)
    ...
    ckpt_path    = os.path.join(args.save_dir, f'karmat_{args.dataset}_best.pt')
    results_path = os.path.join(args.save_dir, f'karmat_{args.dataset}_results.json')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            # Hybrid embeddings — pass entity IDs for transductive component
            ...
            # Negative: score against random entity_table embeddings
            ...
            # Positive via entity_table (aligned with neg scoring)
    ...
    print_results(test_results, split=f'Test [{args.dataset}] KARMA-T')
