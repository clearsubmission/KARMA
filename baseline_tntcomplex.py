...
class TNTComplEx(nn.Module):
    """
    TNTComplEx: Temporal ComplEx with time-entity interaction.
    Lacroix et al. ICLR 2020.
    Score = Re(<s, r, conj(o), t>)
    """
    def __init__(self, num_entities, num_relations, num_times, dim=200):
        ...
        self.time_re = nn.Embedding(num_times,     dim)
        self.time_im = nn.Embedding(num_times,     dim)
        # Time-entity interaction
        self.time_ent = nn.Embedding(num_times,    dim)

        for emb in [self.ent_re, self.ent_im, self.rel_re,
                    self.rel_im, self.time_re, self.time_im, self.time_ent]:
            nn.init.xavier_uniform_(emb.weight)

    def score_all(self, s, r, t):
        """Returns (B, N) scores against all entities."""
        s_re = self.ent_re(s) * (1 + self.time_ent(t))
        s_im = self.ent_im(s) * (1 + self.time_ent(t))
        r_re = self.rel_re(r)
        r_im = self.rel_im(r)
        t_re = self.time_re(t)
        t_im = self.time_im(t)

        # query = s * r * t in complex space
        q_re = s_re*r_re*t_re - s_im*r_im*t_re - s_re*r_im*t_im - s_im*r_re*t_im
        q_im = s_re*r_re*t_im - s_im*r_im*t_im + s_re*r_im*t_re + s_im*r_re*t_re

        # Score = Re(q * conj(o)) = q_re*o_re + q_im*o_im
        all_re = self.ent_re.weight  # (N, d)
        all_im = self.ent_im.weight  # (N, d)

        scores = q_re @ all_re.t() + q_im @ all_im.t()  # (B, N)
        return scores

    def score_triple(self, s, r, o, t):
        s_re = self.ent_re(s) * (1 + self.time_ent(t))
        s_im = self.ent_im(s) * (1 + self.time_ent(t))
        r_re = self.rel_re(r); r_im = self.rel_im(r)
        t_re = self.time_re(t); t_im = self.time_im(t)
        o_re = self.ent_re(o); o_im = self.ent_im(o)

        score = (s_re*r_re*t_re*o_re - s_im*r_im*t_re*o_re
               - s_re*r_im*t_im*o_re - s_im*r_re*t_im*o_re
               + s_re*r_re*t_im*o_im - s_im*r_im*t_im*o_im
               + s_re*r_im*t_re*o_im + s_im*r_re*t_re*o_im)
...
def evaluate(model, quads, filter_dict, device, unseen_entities=None):
    ...
    loader = DataLoader(QuadDataset(quads), batch_size=128, shuffle=False)
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in loader:
            ...
            # Positive scores
            pos = model.score_triple(s, r, o, t)

            # Negative: corrupt object
            neg_o  = torch.randint(0, N, (B,), device=device)
            neg    = model.score_triple(s, r, neg_o, t)

            # Margin loss + L2 reg
            loss = torch.clamp(1.0 - pos + neg, min=0).mean()

            # Regularization on embeddings
            reg = (model.ent_re(s).norm(p=2)**2 +
                   model.ent_im(s).norm(p=2)**2 +
                   model.ent_re(o).norm(p=2)**2 +
                   model.ent_im(o).norm(p=2)**2 +
                   model.rel_re(r).norm(p=2)**2 +
                   model.rel_im(r).norm(p=2)**2 +
                   model.time_re(t).norm(p=2)**2 +
                   model.time_im(t).norm(p=2)**2) / B
    ...
    print(f"\n{'='*50}")
    print(f"  TNTComplEx Results [{args.dataset}]")
    print(f"{'='*50}")
    ...
    with open(f'results/tntcomplex_{args.dataset}.json','w') as f:
        json.dump({'dataset':args.dataset,'test':tr},f,indent=2)
    print(f"Saved: results/tntcomplex_{args.dataset}.json")
